...
function lex(text) {
  ...
  return assert.returnType((new Lexer().tokenize(text)), List);
}
  ...
  describe('lexer', function() {
    describe('token', function() {
      it('should tokenize a simple identifier', function() {
        var tokens = assert.type(lex("j"), assert.genericType(List, int));
        expect(tokens.length).toEqual(1);
        expectIdentifierToken(tokens[0], 0, 'j');
      });
      it('should tokenize a dotted identifier', function() {
        var tokens = assert.type(lex("j.k"), assert.genericType(List, int));
      ...
      it('should tokenize numbers', function() {
        var tokens = assert.type(lex("88"), assert.genericType(List, int));
        expect(tokens.length).toEqual(1);
      ...
      it('should tokenize a string', function() {
        var tokens = assert.type(lex("j-a.bc[22]+1.3|f:'a\\\'c':\"d\\\"e\""), assert.genericType(List, Token));
        expectIdentifierToken(tokens[0], 0, 'j');
        expectOperatorToken(tokens[1], 1, '-');
      ...
      it('should tokenize function invocation', function() {
        var tokens = assert.type(lex("a()"), assert.genericType(List, Token));
        expectIdentifierToken(tokens[0], 0, 'a');
        expectCharacterToken(tokens[1], 1, '(');
        expectCharacterToken(tokens[2], 2, ')');
      });
      it('should tokenize simple method invocations', function() {
        var tokens = assert.type(lex("a.method()"), assert.genericType(List, Token));
        expectIdentifierToken(tokens[2], 2, 'method');
      });
      it('should tokenize method invocation', function() {
        var tokens = assert.type(lex("a.b.c (d) - e.f()"), assert.genericType(List, Token));
        expectIdentifierToken(tokens[0], 0, 'a');
        expectCharacterToken(tokens[1], 1, '.');
        expectIdentifierToken(tokens[2], 2, 'b');
        expectCharacterToken(tokens[3], 3, '.');
      ...
      it('should tokenize number', function() {
        var tokens = assert.type(lex("0.5"), assert.genericType(List, Token));
        expectNumberToken(tokens[0], 0, 0.5);
      });
      it('should tokenize number with exponent', function() {
        var tokens = assert.type(lex("0.5E-10"), assert.genericType(List, Token));
        expect(tokens.length).toEqual(1);
      ...
      it('should tokenize number starting with a dot', function() {
        var tokens = assert.type(lex(".5"), assert.genericType(List, Token));
        expectNumberToken(tokens[0], 0, 0.5);
      });
...
//# sourceMappingURL=/Users/cburgdorf/Documents/hacking/angular/modules/angular2/test/change_detection/parser/lexer_spec.map
